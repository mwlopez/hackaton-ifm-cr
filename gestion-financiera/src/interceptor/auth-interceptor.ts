        ...
        if (req.method === 'GET') {
          ...
          if (data.length > 1) {
            clonedHeaders = clonedHeaders.clone({
              url: data[0] + '?data=' + SimpleBase.base58.encode(EncryptSessionStorage.encrypt(data[1]))
            });
          }
          ...
          if (!environment.localhost) {
              clonedHeaders = clonedHeaders.clone({
              body: { data: SimpleBase.base58.encode(EncryptSessionStorage.encrypt(e)) }
            });
          }
       // }
      }
      return next.handle(clonedHeaders).do((e) => {
      }).catch(response => {
